...
static esp_err_t lan86xx_advertise_pause_ability(esp_eth_phy_t *phy, uint32_t ability) { return ESP_ERR_NOT_SUPPORTED; }

static esp_err_t lan86xx_set_speed(esp_eth_phy_t *phy, eth_speed_t speed) { return ESP_ERR_NOT_SUPPORTED; }

static esp_err_t lan86xx_set_duplex(esp_eth_phy_t *phy, eth_duplex_t duplex) { return ESP_ERR_NOT_SUPPORTED; }
...
static esp_err_t lan86xx_custom_ioctl(esp_eth_phy_t *phy, int cmd, void *data) {
  ...
  switch (cmd) {
    case LAN86XX_ETH_CMD_S_EN_PLCA:
      ...
      if (plca_en) {
        ...
        ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, phy_802_3->addr, ETH_PHY_BMCR_REG_ADDR, &(bmcr.val)), err, TAG,
                          "read BMCR failed");
        ESP_GOTO_ON_FALSE(bmcr.en_loopback == false, ESP_ERR_INVALID_STATE, err, TAG,
                          "PLCA can't be enabled at the same time as loopback");
      }
      ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_read_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                            ETH_PHY_PLCA_CTRL0_REG_MMD_ADDR, &plca_ctrl0.val),
                        err, TAG, "read PLCA_CTRL0 failed");
      plca_ctrl0.en = (*(bool *) data != false);  // anything but 0 will be regarded as true
      ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_write_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                             ETH_PHY_PLCA_CTRL0_REG_MMD_ADDR, plca_ctrl0.val),
                        err, TAG, "write PLCA_CTRL0 failed");
      break;
    ...
    case LAN86XX_ETH_CMD_S_PLCA_NCNT:
      ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_read_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                            ETH_PHY_PLCA_CTRL1_REG_MMD_ADDR, &plca_ctrl1.val),
                        err, TAG, "read PLCA_CTRL1 failed");
      plca_ctrl1.ncnt = *((uint8_t *) data);
      ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_write_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                             ETH_PHY_PLCA_CTRL1_REG_MMD_ADDR, plca_ctrl1.val),
                        err, TAG, "write PLCA_CTRL1 failed");
      break;
    ...
    case LAN86XX_ETH_CMD_S_PLCA_TOT:
      ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_read_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                            ETH_PHY_PLCA_TOTMR_REG_MMD_ADDR, &plca_totmr.val),
                        err, TAG, "read PLCA_TOTMR failed");
      plca_totmr.totmr = *((uint8_t *) data);
      ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_write_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                             ETH_PHY_PLCA_TOTMR_REG_MMD_ADDR, plca_totmr.val),
                        err, TAG, "write PLCA_TOTMR failed");
      break;
    ...
    case LAN86XX_ETH_CMD_ADD_TX_OPPORTUNITY:
      ...
      for (uint16_t i = 0; i < 4; i++) {
        ESP_GOTO_ON_ERROR(
            esp_eth_phy_802_3_read_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE, ETH_PHY_PLCA_MULTID_BASE_MMD_ADDR + i,
                                                &plca_multiple_id_reg.val),
            err, TAG, "read MULTID%d failed", i);
        for (uint8_t j = 0; j < 2; j++) {
          if (plca_multiple_id_reg.entries[j] == 0x00 || plca_multiple_id_reg.entries[j] == 0xff) {
            ...
            ESP_GOTO_ON_ERROR(
                esp_eth_phy_802_3_write_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                     ETH_PHY_PLCA_MULTID_BASE_MMD_ADDR + i, plca_multiple_id_reg.val),
                err, TAG, "write MULTID%d failed", i);
            return ESP_OK;
          }
        }
      }
      ESP_GOTO_ON_FALSE(false, ESP_ERR_NO_MEM, err, TAG,
                        "Unable to add additional transmit opportunity for 0x%02x. Maximum amount (8) reached.",
                        *((uint8_t *) data));
      break;
    case LAN86XX_ETH_CMD_RM_TX_OPPORTUNITY:
      ...
      ESP_GOTO_ON_FALSE(
          false, ESP_ERR_NOT_FOUND, err, TAG,
          "Unable to remove additional transmit opportunity for 0x%02x since it doesn't have one already.",
          *((uint8_t *) data));
      break;
    case LAN86XX_ETH_CMD_S_MAX_BURST_COUNT:
      ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_read_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                            ETH_PHY_PLCA_BURST_REG_MMD_ADDR, &plca_burst_reg.val),
                        err, TAG, "read PLCA_BURST failed");
      plca_burst_reg.maxbc = *((uint8_t *) data);
      ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_write_mmd_register(phy_802_3, MISC_REGISTERS_DEVICE,
                                                             ETH_PHY_PLCA_BURST_REG_MMD_ADDR, plca_burst_reg.val),
                        err, TAG, "write PLCA_BURST failed");
      break;
...
static esp_err_t lan86xx_loopback(esp_eth_phy_t *phy, bool enable) {
  ...
  ESP_GOTO_ON_FALSE(plca_status == false, ESP_ERR_INVALID_STATE, err, TAG,
                    "Unable to set loopback while PLCA is enabled. Disable it to use loopback");
...
esp_eth_phy_t *esp_eth_phy_new_lan86xx(const eth_phy_config_t *config) {
  ...
  ESP_GOTO_ON_FALSE(esp_eth_phy_802_3_obj_config_init(&lan86xx->phy_802_3, config) == ESP_OK, NULL, err, TAG,
