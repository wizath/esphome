...
#define LAN865X_RX_BUFFER_SIZE \
  (((ETH_MAX_PACKET_SIZE + LAN865X_DATA_BLOCK_SIZE - 1) / LAN865X_DATA_BLOCK_SIZE) * LAN865X_DATA_BLOCK_SIZE + \
   LAN865X_HEADER_FOOTER_SIZE)
...
static inline bool lan865x_spi_unlock(emac_lan865x_t *emac) { return xSemaphoreGive(emac->spi_lock) == pdTRUE; }
...
static void *lan865x_spi_init(const void *spi_config) {
  ...
  ESP_GOTO_ON_FALSE(devcfg.command_bits == 0 && devcfg.address_bits == 0, NULL, err, TAG,
                    "incorrect SPI frame format (command_bits/address_bits)");
  ...
  ESP_GOTO_ON_FALSE(spi_bus_add_device(lan865x_config->spi_host_id, &devcfg, &spi_info->hdl) == ESP_OK, NULL, err_spi,
...
static esp_err_t lan865x_frame_transmit(emac_lan865x_t *emac, uint8_t *frame, uint32_t length) {
  ...
  for (int i = 0; i < chunks; i++) {
    ...
    ESP_GOTO_ON_ERROR(emac->spi.read(emac->spi.ctx, 0, 0, tx_block, sizeof(lan865x_tx_block_t)), err_inv_data, TAG,
                      "spi failed");
    ...
    ESP_GOTO_ON_FALSE(footer_parity == rx_block->footer.parity, ESP_ERR_INVALID_CRC, err_inv_data, TAG,
                      "footer parity mismatch");
...
static esp_err_t lan865x_control_transaction(emac_lan865x_t *emac, bool write, uint8_t mms, uint16_t addr, uint8_t len,
                                             uint32_t *data) {
  esp_err_t ret = ESP_OK;
  ...
  // Invalid footer parity indicates potential data corruption at SPI from LAN865X
  ESP_GOTO_ON_FALSE(footer_parity == control_resp->header.parity, ESP_ERR_INVALID_CRC, err, TAG,
                    "footer parity mismatch");
  // Header bad indicates potential data corruption at SPI to LAN865X
...
static esp_err_t lan865x_set_reg_bits(emac_lan865x_t *emac, uint8_t mms, uint16_t addr, uint32_t bitmask) {
  ...
  ESP_RETURN_ON_ERROR(lan865x_read_reg(emac, mms, addr, &reg_value), TAG,
                      "Failed to read register MMS: %" PRIu8 ", ADDR: 0x%04" PRIx16, mms, addr);
  reg_value |= bitmask;
  ESP_RETURN_ON_ERROR(lan865x_write_reg(emac, mms, addr, reg_value), TAG,
                      "Failed to write register MMS: %" PRIu8 ", ADDR: 0x%04" PRIx16, mms, addr);
  return ret;
}
...
static esp_err_t lan865x_indirect_read(emac_lan865x_t *emac, uint8_t addr, uint8_t mask, uint8_t *value) {
  esp_err_t ret = ESP_OK;
  uint32_t reg_value;
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, 0x04, 0x00D8, addr), err, TAG,
                    "Failed to write register MMS: 0x04, ADDR: 0x00D8");
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, 0x04, 0x00DA, 0x2), err, TAG,
                    "Failed to write register MMS: 0x04, ADDR: 0x00DA");
  ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, 0x04, 0x00D9, &reg_value), err, TAG,
                    "Failed to read register MMS: 0x04, ADDR: 0x00D9");
...
static esp_err_t lan865x_reset(emac_lan865x_t *emac) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, LAN865X_MMS_OA, LAN865X_OA_RESET_REG_ADDR, oa_reset.val), err, TAG,
                    "OA_RESET configuration failed");
  // wait for reset complete
  uint32_t to = 0;
  for (; to < LAN865X_SW_RESET_TIMEOUT_MS; to += 10) {
    ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, LAN865X_MMS_OA, LAN865X_OA_RESET_REG_ADDR, &oa_reset.val), err, TAG,
                      "OA_RESET read failed");
  ...
  for (; to < LAN865X_SW_RESET_TIMEOUT_MS; to += 10) {
    ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, LAN865X_MMS_OA, LAN865X_OA_STATUS0_REG_ADDR, &oa_status0.val), err, TAG,
                      "OA_STATUS0 read failed");
  ...
  oa_status0.resetc = 1;
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, LAN865X_MMS_OA, LAN865X_OA_STATUS0_REG_ADDR, oa_status0.val), err, TAG,
                    "OA_STATUS0 configuration failed");
...
static esp_err_t lan865x_verify_id(emac_lan865x_t *emac) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, LAN865X_MMS_MISC, LAN865X_DEVID_REG_ADDR, &devid.val), err, TAG,
                    "Failed to read devid");
  ESP_GOTO_ON_FALSE((devid.model == 0x8650 || devid.model == 0x8651), ESP_ERR_INVALID_VERSION, err, TAG,
                    "Invalid chip ID: 0x%04x", devid.model);
...
static esp_err_t emac_lan865x_start(esp_eth_mac_t *mac) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_set_reg_bits(emac, LAN865X_MMS_MAC, LAN865X_MAC_NCR_REG_ADDR, mac_ncr_mask.val), err, TAG,
                    "MAC_NCR configuration failed");
err:
  return ret;
}
...
static esp_err_t emac_lan865x_stop(esp_eth_mac_t *mac) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_clear_reg_bits(emac, LAN865X_MMS_MAC, LAN865X_MAC_NCR_REG_ADDR, mac_ncr_mask.val), err, TAG,
                    "MAC_NCR configuration failed");
err:
  return ret;
}
...
static esp_err_t emac_lan865x_write_phy_reg(esp_eth_mac_t *mac, uint32_t phy_addr, uint32_t phy_reg,
                                            uint32_t reg_value) {
  esp_err_t ret = ESP_OK;
  emac_lan865x_t *emac = __containerof(mac, emac_lan865x_t, parent);
...
static esp_err_t emac_lan865x_read_phy_reg(esp_eth_mac_t *mac, uint32_t phy_addr, uint32_t phy_reg,
                                           uint32_t *reg_value) {
  esp_err_t ret = ESP_OK;
  emac_lan865x_t *emac = __containerof(mac, emac_lan865x_t, parent);
...
static esp_err_t emac_lan865x_set_addr(esp_eth_mac_t *mac, uint8_t *addr) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, LAN865X_MMS_MAC, LAN865X_MAC_SAB1_REG_ADDR, mac_sab1.val), err, TAG,
                    "MAC_SAB1 configuration failed");
  lan865x_mac_sat1_reg_t mac_sat1 = {
      .val = addr[4] | addr[5] << 8,
  };
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, LAN865X_MMS_MAC, LAN865X_MAC_SAT1_REG_ADDR, mac_sat1.val), err, TAG,
                    "MAC_SAT1 configuration failed");
err:
  return ret;
}
...
static esp_err_t emac_lan865x_get_addr(esp_eth_mac_t *mac, uint8_t *addr) {
  ...
  lan865x_mac_sab1_reg_t mac_sab1;
  ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, LAN865X_MMS_MAC, LAN865X_MAC_SAB1_REG_ADDR, &mac_sab1.val), err, TAG,
                    "MAC_SAB1 read failed");
  addr[0] = mac_sab1.val & 0xFF;
  addr[1] = (mac_sab1.val >> 8) & 0xFF;
  addr[2] = (mac_sab1.val >> 16) & 0xFF;
  addr[3] = (mac_sab1.val >> 24) & 0xFF;
  lan865x_mac_sat1_reg_t mac_sat1;
  ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, LAN865X_MMS_MAC, LAN865X_MAC_SAT1_REG_ADDR, &mac_sat1.val), err, TAG,
                    "MAC_SAT1 read failed");
...
static esp_err_t lan865x_hash_filter_modify(emac_lan865x_t *emac, uint8_t *addr, bool add) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, LAN865X_MMS_MAC, LAN865X_MAC_HRB_REG_ADDR, &mac_hrb.val), err, TAG,
                    "read MAC_HRB register failed");
  ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, LAN865X_MMS_MAC, LAN865X_MAC_HRT_REG_ADDR, &mac_hrt.val), err, TAG,
                    "read MAC_HRT register failed");
  ...
  // Write order matters
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, LAN865X_MMS_MAC, LAN865X_MAC_HRB_REG_ADDR, mac_hrb.val), err, TAG,
                    "write MAC_HRB register failed");
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, LAN865X_MMS_MAC, LAN865X_MAC_HRT_REG_ADDR, mac_hrt.val), err, TAG,
                    "write MAC_HRT register failed");
...
static esp_err_t emac_lan865x_set_link(esp_eth_mac_t *mac, eth_link_t link) {
  ...
  switch (link) {
    case ETH_LINK_UP:
      ...
      if (emac->poll_timer) {
        ESP_GOTO_ON_ERROR(esp_timer_start_periodic(emac->poll_timer, emac->poll_period_ms * 1000), err, TAG,
                          "start poll timer failed");
      }
      break;
    case ETH_LINK_DOWN:
...
static esp_err_t emac_lan865x_set_promiscuous(esp_eth_mac_t *mac, bool enable) {
  ...
  if (enable) {
    ESP_GOTO_ON_ERROR(lan865x_set_reg_bits(emac, LAN865X_MMS_MAC, LAN865X_MAC_NCFGR_REG_ADDR, mac_ncfgr_mask.val), err,
                      TAG, "MAC_NCFGR configuration failed");
  } else {
    ESP_GOTO_ON_ERROR(lan865x_clear_reg_bits(emac, LAN865X_MMS_MAC, LAN865X_MAC_NCFGR_REG_ADDR, mac_ncfgr_mask.val),
                      err, TAG, "MAC_NCFGR configuration failed");
  }
err:
  return ret;
}
...
static esp_err_t emac_lan865x_transmit(esp_eth_mac_t *mac, uint8_t *buf, uint32_t length) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, LAN865X_MMS_OA, LAN865X_OA_BUFSTS_REG_ADDR, &oa_bufsts.val), err, TAG,
                    "OA_BUFSTS read failed");
  ...
  ESP_GOTO_ON_FALSE(lan865x_frame_transmit(emac, buf, length) == ESP_OK, ESP_FAIL, err, TAG,
                    "frame transmit failed at SPI");
err:
  return ret;
}
...
static esp_err_t emac_lan865x_receive(esp_eth_mac_t *mac, uint8_t *buf, uint32_t *length) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_frame_receive(emac, emac->rx_buffer, &frame_len, NULL), err, TAG,
                    "frame receive failed at SPI");
  if (frame_len > 0) {
...
static esp_err_t emac_lan865x_init(esp_eth_mac_t *mac) {
  ...
  ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, LAN865X_MMS_OA, LAN865X_OA_CONFIG0_REG_ADDR, oa_config0.val), err, TAG,
                    "OA_CONFIG0 configuration failed");

  // Clean reset status
  lan865x_oa_status0_reg_t oa_status0_mask = {
      .resetc = 1,
  };
  ESP_GOTO_ON_ERROR(lan865x_clear_reg_bits(emac, LAN865X_MMS_OA, LAN865X_OA_STATUS0_REG_ADDR, oa_status0_mask.val), err,
                    TAG, "OA_STATUS0 configuration failed");
...
esp_eth_mac_t *esp_eth_mac_new_lan865x(const eth_lan865x_config_t *lan865x_config, const eth_mac_config_t *mac_config) {
  ...
  ESP_GOTO_ON_FALSE((lan865x_config->int_gpio_num >= 0) != (lan865x_config->poll_period_ms > 0), NULL, err, TAG,
  ...
  if (lan865x_config->custom_spi_driver.init != NULL && lan865x_config->custom_spi_driver.deinit != NULL &&
    ...
    ESP_GOTO_ON_FALSE((emac->spi.ctx = emac->spi.init(lan865x_config->custom_spi_driver.config)) != NULL, NULL, err,
    ...
    /* SPI device init */
    ESP_GOTO_ON_FALSE((emac->spi.ctx = emac->spi.init(lan865x_config)) != NULL, NULL, err, TAG,
  ...
  if (emac->int_gpio_num < 0) {
    const esp_timer_create_args_t poll_timer_args = {
        .callback = lan865x_poll_timer, .name = "emac_spi_poll_timer", .arg = emac, .skip_unhandled_events = true};
    ESP_GOTO_ON_FALSE(esp_timer_create(&poll_timer_args, &emac->poll_timer) == ESP_OK, NULL, err, TAG,
                      "create poll timer failed");
  }
